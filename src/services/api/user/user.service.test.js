...
describe('UserService', () => {
  ...
  it('checkCurrentUser returns current user', async () => {
    server.use(
      rest.get(`${BASE_URL}/currentuser`, (req, res, ctx) =>
        res(ctx.json({ message: 'Current user', user: { username: 'manny' } }))
      )
    );
  ...
  it('searchUsers returns matching users', async () => {
    server.use(
      rest.get(`${BASE_URL}/user/profile/search/:query`, (req, res, ctx) =>
        res(ctx.json({ message: 'Search results', users: [] }))
      )
    );
  ...
  it('updateNotificationSettings returns updated settings', async () => {
    server.use(
      rest.put(`${BASE_URL}/user/profile/settings`, (req, res, ctx) =>
        res(ctx.json({ message: 'Settings updated', settings: {} }))
      )
    );
  ...
  it('updateBasicInfo returns updated info', async () => {
    server.use(
      rest.put(`${BASE_URL}/user/profile/basic-info`, (req, res, ctx) =>
        res(ctx.json({ message: 'Basic info updated' }))
      )
    );
  ...
  it('updateSocialLinks returns updated links', async () => {
    server.use(
      rest.put(`${BASE_URL}/user/profile/social-links`, (req, res, ctx) =>
        res(ctx.json({ message: 'Social links updated' }))
      )
    );
